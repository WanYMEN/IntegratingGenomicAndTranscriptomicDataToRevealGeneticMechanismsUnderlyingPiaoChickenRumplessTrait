...
labeledHeatmap(Matrix = as.matrix(moduleTraitCor), xLabels = c("Piao"), yLabels = paste("ME", rowColors, sep=""), colorLabels = FALSE, ySymbols = names(moduleTraitCor), colors = greenWhiteRed(50), textMatrix = textMatrix, setStdMargins = FALSE, cex.text = 1, zlim = c(-1, 1), main = paste("Module-trait relationships"), xLabelsAngle = 0)
dev.off()
